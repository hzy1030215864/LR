...
loaded_df = pd.read_excel("E:\代码\lg\data\S_jt_24.xlsx")
# 将数据转换回矩阵（NumPy数组）
S_jt = loaded_df.to_numpy()
loaded_df1 = pd.read_excel("E:\代码\lg\data\S_it_24.xlsx")
# 将数据转换回矩阵（NumPy数组）
...
for j in range(J):
    for t in range(T):
        ...
        F_jt[j, t] = model.addVar(vtype=GRB.CONTINUOUS,name=f"F_{j}_{t}",lb=0)
...
for j in range(J):
    for t in range(T):
        model.addConstr(F_jt[j,t]<=O[j,t]-V_jt[j, t])
for j in range(J):
...
for t in range(1,T):
    ...
    model.addConstr(lhs_heat <= 412450000)
    model.addConstr(lhs_heat >= 0)
for j in range(J):
    for t in range(1,T):
        lambda_i1jt1[j,t]= 0
z = {}
...
for iteration in range(max_iterations):
    ...
    for t in range(1, T):
        ...
        objective_expr += lambda_i1jt1[j, t] * (h_jt[j, t - 1] + O[j, t] - H_j0[j] - V_jt[j, t] - F_jt[j, t])
        for i1 in range(I1):
            objective_expr.addTerms([-lambda_i1jt1[j, t]], [l_i1jt[i1, j, t]])
        for i2 in range(I2):
            objective_expr.addTerms([-lambda_i1jt1[j, t]], [l_i2jt[i2, j, t]])
    # 设置最小化目标
    ...
    for j in range(J):
        for t in range(1, T):
            erlfa1[j, t] = O_value[j, t] + h_value[j, t - 1] - h_value[j, t] - V_value[j, t] - F_value[j, t]
            for i1 in range(I1):
                erlfa1[j, t] -= l1_value[i1, j, t]
            for i2 in range(I2):
                erlfa1[j, t] -= l2_value[i2, j, t]
    for j in range(J):
        for t in range(1, T):
            if erlfa1[j, t] == 0:
                lambda_i1jt1[j, t] = lambda_i1jt1[j, t]
            else:
                objective_value = objective_expr.getValue()
                sete[j, t] = (441334 - objective_value) / (erlfa1[j, t] * erlfa1[j, t])
                fm = erlfa1[j, t]
                lambda_i1jt1[j, t] += sete[j, t] * fm
